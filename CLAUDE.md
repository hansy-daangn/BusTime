...
1. **실 API 데이터만.** Mock/가짜 데이터 절대 금지.
   극도로 깔끔·단순·심리스 지향 — 필수 기능에 필요 없는 요소·파일·설정은 제거한다.
...
## 파일 구조 (2026-07-04 대청소 — 이게 전부다)

| 파일 | 역할 |
|---|---|
| `app.html` | 위젯 본체 (단일 파일: CSS+JS 인라인 + 서울 노선 인덱스 내장). **DEV 마커 규약** 참고 |
| `data/seoul/route-*.json` | 서울 노선별 정류소 정적 스냅샷 (718개) |
| `.github/workflows/pages.yml` | main 푸시 시 사용자/개발자 빌드 + data/ 배포 |
| `README.md`, `CLAUDE.md` | 사용자용 초간단 / 작업 맥락 전체 |

- **proxy.py 삭제됨** — TAGO가 HTTPS+CORS 직접 호출이라 중계 불필요. 서울 도착(HTTP)은 앱 내 relay(allorigins) 폴백.
- **index.html(랜딩)·designs.html 삭제됨** — 배포 빌드의 index.html은 app.html에서 생성된다.
- 연결 모드·프록시 URL·보조 키 설정도 전부 제거. 남은 개발자 설정은 서비스 키 1개.
...
- 현재 DEV 블록 4개: 헤더 도움말 버튼, 설정의 "개발자"(키) 섹션, 도움말 페이지, 하단 시나리오 탭.
...
- 키는 **1개만** 내장 (보조 키 제거됨). 415a… 키는 열린데이터광장(openapi.seoul.go.kr:8088) 전용 — 정적 데이터 재생성에만 쓰임.
- 호출 체인: **direct → relay(allorigins)** 2단 고정. API가 응답한 오류(키인증 등)는 경로 재시도 없이 즉시 반환.
...
## 알려진 제약 / 외부 대기 (2026-07-04 기준)

- **서울 도착정보**: 키(103d…)가 서울 인증서버(ws.bus.go.kr)에 미등록 — "서울특별시_버스도착정보조회" 승인 전파 대기. 코드는 완성돼 있어 전파되면 즉시 작동. 세션 크론이 매시간 감시.
- **인천·경기 노선 검색**: TAGO 버스노선정보/버스정류소정보 활용신청 미완(403) — data.go.kr 계정에서 신청해야 함(자동승인). 기본 9802·논현역은 ID 내장이라 무관.
- 서울 도착은 API가 HTTP뿐이라 Pages에서 relay(공용 릴레이) 경유 — 릴레이 다운 시 서울 도착만 일시 불가. Electron 단계에서 소멸.
